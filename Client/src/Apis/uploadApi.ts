...
export type ProgressMapSetter = React.Dispatch<React.SetStateAction<Record<string, any>>>;
export type ModalShower = (title: string, message: string, type?: string) => void;
...
const uploadFileSimple = async (
  ...
  setProgressMap: ProgressMapSetter,
  showModal: ModalShower
) => {
  try {
...
const uploadFileMultipart = async (
  ...
  let uploadData: ApiResponse | undefined;
  try {
...
// API calls
const uploadFileInitiate = async (fileData: any): Promise<ApiResponse> => {
  try {
...
const getPartPresignedURL = async ({ fileId, partNumber }: { fileId: string; partNumber: number }): Promise<ApiResponse> => {
  try {
...
const uploadComplete = async (fileId: string, parts: Array<{ partNumber: number; eTag: string }>): Promise<ApiResponse> => {
  try {
...
const abortUpload = async (fileId: string): Promise<ApiResponse> => {
  try {
