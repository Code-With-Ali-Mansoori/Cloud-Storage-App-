...
export interface ImportFromDriveProps {
...
export default function ImportFromDrive({
  ...
  async function handleOpen() {
    ...
    openPicker({
      ...
      viewId: "DOCS",
      supportDrives: true,
      callbackFunction: (data: any) => handlePicked(data, token),
    });
  }
  ...
  async function handlePicked(data: any, token: string) {
    ...
    try {
      ...
      for (const f of files) {
        ...
        if (!res.success) {
          reset();
          showModal("Error", res.message || "Failed to import file", "error");
          return;
        }
        step(1);
      }
