...
const UploadSection: React.FC<UploadSectionProps> = ({ setShowCreateModal, setActionDone }) => {
  ...
  const handleFileUpload = async (selectedFiles: FileList | File[] | null) => {
    ...
    const userMaxLimit = user?.maxFileSize || 524288000;

    // checking the file size before initiating uploads.
    for (const file of fileList) {
      if (file.size > userMaxLimit) {
        showModal(
