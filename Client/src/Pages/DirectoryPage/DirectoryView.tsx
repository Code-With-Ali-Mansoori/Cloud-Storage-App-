...
const DirectoryView: React.FC<DirectoryViewProps> = ({
  ...
  const [detailsModal, setDetailsModal] = useState<{ isOpen: boolean; item: any }>({
    isOpen: false,
    item: null,
  });
  ...
  const filteredAndSortedItems = useMemo(() => {
    const filtered = allItems.filter((item: DirectoryItem) =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const multiplier = sortOrder === "asc" ? 1 : -1;

    const getKey = (item: DirectoryItem) => {
      switch (sortBy) {
        case "size":
          return item.size ?? 0;
        case "modified":
  ...
  const handleDetailsOpen = (item: DirectoryItem) => {
