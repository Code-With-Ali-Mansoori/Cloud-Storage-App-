...
export const createThrottler = (
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let key = `${throttlerName}:${clientIp.replace(/:/g, "_")}`;
      const user = (req as any).user;
      if (useUserId && user?._id) {
        key = `${throttlerName}:${user._id.toString()}`;
      }

      const now = Date.now();
      const requestDataRaw = await redisClient.get(key);
      let { count, previousDelay, lastRequestTime } = requestDataRaw
        ? JSON.parse(typeof requestDataRaw === "string" ? requestDataRaw : requestDataRaw.toString())
        : {
            count: 0,
            previousDelay: 0,
...
export const throttler: Record<string, RequestHandler> = Object.fromEntries(
  Object.entries(throttlerConfig).map(([key, value]) => [
    key,
    createThrottler(key, ...(value as any)),
  ])
);
