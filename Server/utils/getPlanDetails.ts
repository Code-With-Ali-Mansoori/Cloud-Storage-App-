...
const plans: Record<string, PlanDetails> = process.env.PAYMENT_ENV === "test" ? TEST_PLANS : LIVE_PLANS;

export const getPlanDetailsById = (planId: string): PlanDetails | null => {
  if (planId == "default")
    return {
      limits: {
        storage: "500 MB",
...
export const getPlansEligibleForChange = (activePlanId: string): string[] => {
