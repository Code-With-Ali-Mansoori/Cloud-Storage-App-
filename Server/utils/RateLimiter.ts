...
export const RateLimiter = ({
  ...
  keyGenerator = (req: Request, res: Response) => ipKeyGenerator(req.ip || "", 56), // default: IP-based with IPv6 support
}: RateLimiterOptions = {}) => {
  return rateLimit({
...
const makeLimiter = (
  ...
  return RateLimiter({
    keyGenerator: (req: Request, res: Response): string => {
      ...
      if (useUserId && user?._id) return user._id.toString();
...
const rateLimiterConfig = {
    ...
    guestFileAccessLimiter: [60, _10Mins, false, true,  "Too many attempts to access the file. Please wait 10 minutes."],
...
export const limiter: Record<string, RequestHandler> = Object.fromEntries(
