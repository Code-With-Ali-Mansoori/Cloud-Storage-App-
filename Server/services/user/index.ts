...
import { deleteS3Objects, s3Client } from "../file/s3Services";
import { DirectoryServices } from "../index";
import { razorpayInstance } from "../razorpayService";
import Subscription from "../../models/subscriptionModel";

const logoutUserService = async (token: string): Promise<void> => {
  await redisClient.del(`session:${token}`);
};

const logoutAllUserService = async (userId: string): Promise<void> => {
  await redisClient.deleteManySessions(userId);
};

const getAllUserService = async (currentUser: any): Promise<any[]> => {
  ...
  const sessions = await Promise.all(
    keys.map(async (key: string) => ({ data: await redisClient.json.get(key) }))
  );
  const sessionUserIds = new Set(sessions.map((s) => (s.data as any)?.userId));
  ...
  const Users = AllUsers.filter(
    ...
    return {
      ...
      storage: {
        usedStorage: (user.rootDirId as any).size,
...
const logoutSpecificUserService = async (userId: string, currentUser: any): Promise<void> => {
...
const softDeleteUserService = async (userId: string, currentUser: any): Promise<void> => {
  ...
  if (user.subscriptionId) {
    ...
    await razorpayInstance.subscriptions.pause(
      (user.subscriptionId as any).razorpaySubscriptionId,
      { pause_at: "now" }
    );

    // updating the DB
    await Subscription.findByIdAndUpdate((user.subscriptionId as any)._id, {
      status: "paused",
    });
  }
...
const hardDeleteUserService = async (userId: string, currentUser: any): Promise<void> => {
...
const recoverUserService = async (userId: string, currentUser: any): Promise<void> => {
  const user = await User.findOne({ _id: userId })
...
const setPasswordService = async (userId: string, password: string): Promise<void> => {
...
const updatePasswordService = async (userId: string, oldPassword: string, newPassword: string): Promise<void> => {
...
const updateProfileService = async (userId: string, file: any, name: string): Promise<any> => {
...
const deleteUserService = async (userId: string): Promise<void> => {
  ...
  if (user.subscriptionId) {
    try {
      ...
      await Subscription.findByIdAndDelete((user.subscriptionId as any)._id);
    } catch (err) {
...
const changeRoleService = async (newRole: string, currentUser: any, userId: string): Promise<any> => {
  ...
  target_user.role = newRole as any;
  await target_user.save();

  return target_user;
};

const getSpecificUserDirectoryService = async (userId: string, dirId?: string): Promise<any> => {
  const targetUser = await User.findOne({ _id: userId })
    .select("-password")
    .lean();
  const { _id, directory, files, name, parentDirId } =
    await DirectoryServices.GetDirectoryDataService(userId, dirId || undefined);
  return {
    files: files,
...
const getFileService = async (fileId: string, userId: string): Promise<any> => {
