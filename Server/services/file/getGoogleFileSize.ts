...
import { getExportMimeType } from "../../utils/getExtension&MimeType";

export async function getGoogleFileSize(file: any, token: string): Promise<number> {
  try {
    ...
    console.log("size fetch error:", err?.message || err);
    return 0;
  }
}
